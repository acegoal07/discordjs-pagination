...
module.exports = PaginationBase = async({
   portal, pageList, buttonList, timeout = 12000,
   replyMessage = false,
   autoDelete = false,
   privateReply = false,
   ...
   try {
      ...
      if (!selectMenu.toggle) {
         ...
         if (selectMenu.useTitle) {
            ...
            selectMenu.labels = labels;
         }
      }
      // Interaction
      if (new MessagePayload(portal).isInteraction) {
         // Checks
         if (pageList.length < 2) {
            if (privateReply) {
               await portal.deferred ? await portal.editReply("The reply has been sent privately") : await portal.reply({ content: "The reply has been sent privately", ephemeral: ephemeral});
               return portal.client.users.cache.get(portal.member.user.id).send({embeds: [pageList[0]]});
            } else {
               return portal.deferred ? await portal.editReply({embeds: [pageList[0]]}) : await portal.reply({embeds: [pageList[0]]});
            }
         }
         if (portal.ephemeral) {
         ...
         // Run
         return Promise.resolve(InteractionPagination(portal, pageList, buttonList, timeout, autoDelete, privateReply, progressBar, authorIndependent, selectMenu, ephemeral));
      }
      // Message
      // Checks
      ...
      if (!portal.channel) throw new Error("PaginationBase ERROR: Channel is inaccessible");
      if (pageList.length < 2) {
         if (privateReply) {
            await portal.channel.send("The reply has been sent privately");
            return portal.author.send({embeds: [pageList[0]]});
         } else {
            return replyMessage ? portal.reply({embeds: [pageList[0]]}) : portal.channel.send({embeds: [pageList[0]]});
         }
      }
      // Run
      return Promise.resolve(MessagePagination(portal, pageList, buttonList, timeout, replyMessage, autoDelete, privateReply, progressBar, authorIndependent, selectMenu));
   } catch(error) {
