...
// Params
/**
 * The Direct pagination
 *
 * @param {Message} direct - Discord.js interface
 ...
 */
// Direct pagination
module.exports = DirectPagination = async(direct, pageList, buttonList, timeout, autoDelete, progressBar, proSlider, proBar, selectMenu) => {
   try {
      // Set page number
      let pageNumber = 0;
      // Create row
      ...
      let pagination = await direct.author.send(paginationContent);
      // Create collector
      const collector = pagination.createMessageComponentCollector({
         filter: FilterCreator({direct, buttonList, selectMenu}),
         time: timeout
      });
      // Button inputs
      collector.on("collect", async (i) => {
         ...
         if (!selectMenu) {
            switch (i.customId) {
               ...
               // Catch null
               case null:
                  throw new Error("DirectPagination ERROR: The button input is null");
               // Catch undefined
               case undefined:
                  throw new Error("DirectPagination ERROR: The button input is undefined");
               // Default
               default:
                  break;
            }
