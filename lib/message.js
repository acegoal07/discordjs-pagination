...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Message pagination ////////////////////////////////////////////////////////////////////////////////////////////////////
module.exports = MessagePagination = async (message, pages, buttonList, timeout, replyMessage, autoDelete, privateReply, progressBar) => {
   // Set page number
   let page = 0;
   // Create embed
   const row = new MessageActionRow().addComponents(buttonList);
   const curPageContent = {
      embeds: [pages[page].setFooter({text: `${progressBar.toggle ? `${await progressBarBuilder(pages.length, page, progressBar)}` : `Page ${page + 1} / ${pages.length}`}`})],
      components: [row]
   };
   let curPage;
   if (privateReply) {
      curPage = await message.author.send(curPageContent);
      replyMessage ? await message.reply("The reply has been sent privately") : await message.channel.send("The reply has been sent privately");
   } else {
