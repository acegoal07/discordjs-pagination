...
module.exports = InteractionPagination = async(interaction, pageList, buttonList, timeout, autoDelete, privateReply, progressBar, authorIndependent, selectMenu) => {
   try {
      ...
      const paginationContent = {
         embeds: [pageList[pageNumber].setFooter({text: `${progressBar.toggle ? `${ProgressBarCreator(pageList.length, pageNumber, progressBar.slider, progressBar.bar)}` : `Page ${pageNumber + 1} / ${pageList.length}`}`})],
      ...
      collector.on("end", async() => {
         try {
            ...
            if (selectMenu) {
               try {
                  return interaction.editReply({ components: [await DisabledSelectMenuCreator(pageList.length)] });
               } catch(error) {}
            }
            // Disable buttons
            else {
