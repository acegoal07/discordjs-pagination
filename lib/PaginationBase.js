...
const DirectPagination = require("./DirectPagination");
...
module.exports = PaginationBase = async({
   interaction, message, direct, pageList, buttonList, timeout = 12000,
   replyMessage = false,
   autoDelete = false,
   privateReply = false,
   progressBar = false,
   ...
   try {
      // Checks
      if (message && interaction) throw new Error("PaginationBase ERROR: Do not provided both interaction and message only 1 is required");
      if (!pageList) {
         if (pageBuilderInfo) {
      ...
      if (typeof message?.author === "object") {
         ...
         // Run
         return Promise.resolve(MessagePagination(message, pageList, buttonList, timeout, replyMessage, autoDelete, privateReply, progressBar, proSlider, proBar, authorIndependent, selectMenu));
      }

      // Direct
      if (typeof direct?.author === "object") {
         process.emit("You are using an experimental feature which is not tested fully and is still in development")
         // Checks
         if (pageList.length < 2) {
            return direct.author.send({embeds: [pageList[0]]});
         }
         if (replyMessage || privateReply || authorIndependent) {
            process.emit("PaginationBase WARNING: Direct pagination does not support replyMessage, privateReply or authorIndependent");
         }
         // Run
         return Promise.resolve(DirectPagination(direct, pageList, buttonList, timeout, autoDelete, progressBar, proSlider, proBar, selectMenu));
      }
      ...
      // Interaction
         ...
         if (pageList.length < 2) {
            if (privateReply) {
               await interaction.deferred ? await interaction.editReply("The reply has been sent privately") : await interaction.reply({ content: "The reply has been sent privately", ephemeral: ephemeral});
         ...
         // Run
         return Promise.resolve(InteractionPagination(interaction, pageList, buttonList, timeout, autoDelete, privateReply, progressBar, proSlider, proBar, authorIndependent, selectMenu, ephemeral));
      }
