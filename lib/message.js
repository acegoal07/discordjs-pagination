...
module.exports = MessagePagination = async (message, pageList, buttonList, timeout, replyMessage, autoDelete, privateReply, progressBar, proSlider, proBar) => {
   ...
   const curPageContent = {
      embeds: [pageList[pageNumber].setFooter({text: `${progressBar? `${await progressBarBuilder(pageList.length, pageNumber, proSlider, proBar)}` : `Page ${pageNumber + 1} / ${pageList.length}`}`})],
      components: [row]
   };
   let curPage;
   if (privateReply) {
