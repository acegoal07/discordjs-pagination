...
await new Pagination()
   ...
   .setButtons([
      ...
      new PageButtonBuilder()
         ...
         .setCallback(async (paginationSession, interaction) => {
            const modal = new ModalBuilder().setCustomId('goToPageModal').setTitle('Go to Page');
            ...
            const submitted = await interaction.awaitModalSubmit({
               time: 60000,
               filter: (i) => i.customId === 'goToPageModal' && i.user.id === interaction.user.id
            });
            ...
            await paginationSession.goToPage(
               interaction,
               submitted.fields.getTextInputValue('pageInput')
            );
   ...
   .setPages([
      new EmbedPageBuilder().setTitle('Welcome').setDescription('This is an embed page.'),
   ...
   .setPages([
      new ContainerPageBuilder().addTextDisplayComponents((textDisplay) =>
         textDisplay.setContent('this is a container page')
      ),
      new Pagination.TextDisplayPageBuilder().setContent('This is a text display page')
   ])
   .paginate();
```
